<?php
	...
	if(defined('TOKEN_WEBSERVICE')){
		...
		if(constant('TOKEN_WEBSERVICE')==$objConfiguracion->obtenerTokenWebservices()){	
			...
			$objConsultaSQL->addCampoSelect('directores.id',  		 'id');
			$objConsultaSQL->addCampoSelect('directores.nombre',     'nombre');
			$objConsultaSQL->addCampoSelect('directores.apellidos',  'apellidos');
			...
			$objConsultaSQL->addCondicionWhere('directores.id', '= ' . $parametrosRecibidos['id']);
